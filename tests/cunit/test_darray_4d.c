/*
 * Tests for PIO distributed arrays. This test uses 4 dimension; it
 * duplicates a test in fortran test program
 * tests/general/pio_decomp_frame_tests.
 *
 * Ed Hartnett
 * 1/10/18
 */
...
/* The dimension names. */
#define DIM_ROW_NAME "PIO_TF_test_dim_row"
#define DIM_ROW_LEN 2
#define DIM_COL_NAME "PIO_TF_test_dim_col"
#define DIM_COL_LEN 10
#define DIM_HGT_NAME "PIO_TF_test_dim_hgt"
#define DIM_HGT_LEN 2
...
#define NUM_FILLVALUE_PRESENT_TESTS 2
    ...
    int dimid[NDIM4];     /* The dimension ID. */
    ...
    for (int fmt = 0; fmt < num_flavors; fmt++)
    {
        ...
        for (int with_fillvalue = 0; with_fillvalue < NUM_FILLVALUE_PRESENT_TESTS; with_fillvalue++)
        {
            ...
            if ((ret = PIOc_def_dim(ncid, DIM_ROW_NAME, DIM_ROW_LEN, &dimid[3])))
                ERR(ret);
            if ((ret = PIOc_def_dim(ncid, DIM_COL_NAME, DIM_COL_LEN, &dimid[2])))
                ERR(ret);
            if ((ret = PIOc_def_dim(ncid, DIM_HGT_NAME, DIM_HGT_LEN, &dimid[1])))
                ERR(ret);
            if ((ret = PIOc_def_dim(ncid, DIM_TIME_NAME, DIM_TIME_LEN, &dimid[0])))
                ERR(ret);

            /* Define a variable. */
            if ((ret = PIOc_def_var(ncid, VAR_NAME, pio_type, NDIM4, dimid, &varid)))
                ERR(ret);
